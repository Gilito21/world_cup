    ...
    return (
      <>
        <button
          ...
          className="flex items-center gap-1.5 text-xs sm:text-sm px-2.5 sm:px-3 py-1.5 rounded-lg bg-paper-200 border border-ink/20 text-ink hover:bg-paper transition-colors min-h-[36px]"
        >
          <span>+</span>
    ...
    <>
      <div ref={ref} className="relative">
        ...
        >
          ...
          <svg
            className={`w-3 h-3 sm:w-3.5 sm:h-3.5 text-ink/60 flex-shrink-0 transition-transform ${open ? 'rotate-180' : ''}`}
        ...
        {open && (
          <div className="absolute left-1/2 -translate-x-1/2 sm:left-auto sm:translate-x-0 sm:right-0 top-full mt-1.5 w-[min(16rem,calc(100vw-1.5rem))] card shadow-2xl shadow-ink/50 py-1.5 z-40 animate-fade-in">
            <div className="px-3 py-1.5 text-xs font-semibold text-ink/50 uppercase tracking-wider">
              {t('league.yourLeagues')}
            </div>

            {leagues.map(league => (
              <div key={league.id} className={`flex items-center ${activeLeague?.id === league.id ? 'bg-paper' : ''}`}>
                <button
                  onClick={() => { setActiveLeague(league); setOpen(false) }}
                  className="flex-1 flex items-center gap-2.5 px-3 py-2.5 text-left hover:bg-paper transition-colors min-w-0"
                >
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5">
                      {league.role === 'admin' && <span className="text-xs">👑</span>}
                      <span className="text-sm font-medium text-ink truncate">{league.name}</span>
                    </div>
                    {league.role === 'admin' && (
                ...
                {league.role === 'admin' && (
                  <button
                    ...
                    className="flex-shrink-0 px-2.5 py-2.5 text-ink/50 hover:text-ink hover:bg-paper-200 transition-colors rounded-r-lg"
            ...
            <div className="border-t border-ink/20 mt-1.5 pt-1.5">
              <button
                ...
                className="w-full flex items-center gap-2 px-3 py-2 text-sm text-ink hover:bg-paper transition-colors rounded-lg mx-0"
              >
